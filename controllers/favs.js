const { usersModel, charactersModel } = require("../models")
...
const addFav = async (req, res) => {
    try {
        ...
        let character = await charactersModel.findOne({ _id: id })

        const favRepeat = findUser.favs.includes(id) ? true : false

        if (favRepeat) return res.status(404).json({message:"Already a fav"})
...
const deleteFav = async (req, res) => {
    try {
        ...
        const character = await charactersModel.findOne({ _id: id })
